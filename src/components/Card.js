...
const Card = ({imgUrl,price,title,address,beds,bathrooms,area,isPopular,isLiked}) => {
    const [liked,setLiked] = useState(false);
  return (
    <div className={styles.card}>
        <div className={styles.image}>
            <img src={imgUrl} alt="" className='h-[160px] w-full rounded-t-md'/>
            {isPopular ? <div className={styles.popularTag} id="tag"><WiStars size={30}/>POPULAR</div> : null}
        </div>
        
        <div className={styles.details}>
           <div className="flex flex-col">
            <div className={styles.price}>${price}/<span className={styles.text}>month</span></div>
            <div className={styles.title}>{title}</div>
           </div>
           <div className={styles.heartIcon}>{!liked ?<FaRegHeart size={20} color="#4f46e5" onClick={() => { setLiked(!liked) }} /> : <FcLike size={20} color="#4f46e5" onClick={() => { setLiked(!liked) }}/> } </div>
        </div>
        ...
        <div className={styles.information}>
            <div className={styles.infoIcons} style={{gap:2}}>
                <TbBed color='#4f46e5'/>{beds} Beds
            </div>
            <div className={styles.infoIcons} style={{gap:2}}>
                <TbBath color='#4f46e5'/>{bathrooms} Bathrooms
            </div>
            <div className={styles.infoIcons} style={{margin:"1px"}}>
                <TbRotateRectangle color='#4f46e5'/>{area}<sup>2</sup>
            </div>
        </div>
    </div>
  )
